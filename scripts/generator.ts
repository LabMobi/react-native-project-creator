...
const optionalFolders: {
  name: string;
  condition: (libs: string[]) => boolean;
}[] = [
  {
    name: "api",
    condition: (libs) => libs.includes("orval") || libs.includes("react-query"),
  },
  { name: "i18n", condition: (libs) => libs.includes("react-i18next") },
];

const inquire = (callback: (value: unknown) => void) =>
  inquirer.prompt(QUESTIONS).then((answers) => {
    let newPackageJson;
    try {
      const projectPackageJson = JSON.parse(
        fs.readFileSync(PACKAGE_JSON_PATH, { encoding: "utf8" })
      );
      newPackageJson = {
        ...projectPackageJson,
        dependencies: {
          ...projectPackageJson.dependencies,
          ...answers.libraries.reduce(
            (acc, it) => ({ ...acc, [it]: OPTIONAL_LIBRARIES[it] }),
            {}
          ),
        },
      };
